...
5. Symbol repeats at gap 4 are ~2x over-represented; gaps 1-3 suppressed. **Reconfirmed 2026-08-10 after dedup** (the raw count of 26 double-counts shared headers): 20 observed vs a constrained-reshuffle null of 9.95 +- 3.04, z = +3.31, p = 0.0017, clearing Bonferroni over 29 gaps. All 9 messages independently exceed expectation. Gaps 5-30 are all at chance, so the mechanism's memory is ~4 steps. This is the sharpest mechanism fingerprint in the corpus - `tools/gap_spectrum.py`.
...
## Provenance caveat - settle this before more key search

Constraints 1 and 3 (exactly 83 contiguous values; exactly zero adjacent repeats) are both properties of ONE trigram reading order, selected from ~86,000 candidates *because* it yielded a gapless 0-82 range. Regrouping eyes changes which trigrams are adjacent, so constraint 3 is not invariant to that choice and may be partly a selection artifact. If it is, the inference "output must depend on previous output" weakens a lot - and that inference is the only thing making the leading hypothesis (2) look uncomfortable. Testing it needs the **raw per-eye orientation sequences**, which this repo does not have: `data/eye_trigrams.csv` is already decoded. Obtaining them is the highest-value data task open.
