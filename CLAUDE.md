...
6. **Period-4 repeating key component. NEW LEAD 2026-08-10.** The only tested construction that reproduces the gap-4 shape (spike at 4, chance elsewhere), which it does by passing the plaintext's own distance-4 repeats through while scrambling other distances. Simple enough for the author prior: a four-symbol key. Fails constraint 3 on its own (10.9 adjacent repeats per 1027) so it must be a *component*, combined with something that forbids repeats - e.g. c[i] = p[i] + k[i mod 4] + c[i-1], where zero adjacent repeats needs only that the plaintext avoid the four values -k[j]. Untested. Also check gaps 8 and 12, which a pure period-4 key should elevate and the data does not.
